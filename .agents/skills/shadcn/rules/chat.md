...
Install (from `apps/rxlab-web/`): `pnpm dlx shadcn@latest add message-scroller message bubble attachment marker`

The same component names and props ship for both `base` and `radix`; this
project is radix, so triggers compose with `asChild`. See [base-vs-radix.md](./base-vs-radix.md).
...
The RSS pre-paint script from upstream applies to server HTML only; this SPA
fetches client-side, so skip it.
