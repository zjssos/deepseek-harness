...
- `rxlab_collect` domain (version 1 at shipment, per-record layout; v2 later added the optional capture `params` and made version-1 captures readable, see [the Wiki v2 note](2026-09-09-rxlab-wiki-v2-attributes-and-import.md)) with three tables: `links` (assets: platform/shop/sku/url/mobileUrl/status/last-capture summary), `captures` (one record per successful run: title/price(displayed value+raw+note)/selectedSku/buyUrl/capture time), and `batches` (queued run state with per-link items and counts).
