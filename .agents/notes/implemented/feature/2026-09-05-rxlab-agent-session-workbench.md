...
**The host exposes settings/credentials and workspace Remote namespaces.** `packages/bundle/rxlab-app/cordis.patch.yml` activates `dsh-api-settings-controller` and `dsh-api-workspace-controller` rows (dependencies already declared in the bundle), so the SPA reaches `remote.credentials` (describe/set/unset) and `remote.workspace` (archiveSession). The model settings dialog in the workbench header edits the credential the DeepSeek adapter resolves (`DEEPSEEK_API_KEY` by default, ref editable) and writes it into `$DSH_HOME/.credentials.yaml` through `credentials/set`; configured state comes back from `credentials/describe`, and clearing uses `credentials/unset`.

**Session management covers the object layer's remaining verbs.** The row menu archives a session (the workspace-domain archive semantics: row hidden, log and accounting kept). Because rxlab's list rows come from `session-controller.list` while the archive set is a workspace fact, `useArchivedSessions` reads the complete set from the `workspace.follow` baseline once per connection and extends it locally after each accepted archive; archiving the current session clears back to the no-session view. Message rows expose a fork entry (`sessions.fork({ sessionId, atSeq: row.seq, increaseTitle: true })`, opening the child — the host cuts at the first turn/end at or after the seq), a "load older" button renders when `snapshot.hasMore`, queue rows offer remove/steer through `face.updateQueue`, and a header control clears the current selection via `sessions.clear()`.
...
**Reuse the official workspace browser for the session list.** The archive set and workspace grouping live in `ui-workspace` client machinery; rxlab keeps the `session-controller.list` feed and filters archived ids from a one-shot `workspace.follow` baseline read instead of installing the workspace client object layer.

## Consequences

The rxlab SPA now runs real host sessions from the browser: list/create/open/rename/archive/fork, queued sends, stop, queue-item remove/steer, history paging, model catalog selection, and a transcript with tool cards, with no keyless-send crash. The workbench also edits the model credential in place, so a fresh host needs no terminal or Models page to become usable. The module remains independent of the client roster, so it cannot regress on `packages/client` slot contract changes that do not touch the object layer. Streaming (live chunks), attachment upload, queue edit, unarchive, and durable model-projection display remain future work; the conversation is rendered from durable events alone, and client-only pending echoes are not yet displayed beyond the queue count.

## Testing

Browser smoke on `dsh rxlab` verifies the connected badge, persistent list, create/open/rename, archive persistence across reload (and archive-of-current returning to the no-session view), fork opening a titled child, the catalog-backed model selector, credential set/clear round-tripping into `$DSH_HOME/.credentials.yaml`, and a keyless send that fails into the error banner without crashing. `apps/rxlab-web` typecheck and Vite build are green; the host build (`pnpm run build:lib:host`) is unchanged by this work beyond the earlier rows patch already verified in the rxlab profile work.
