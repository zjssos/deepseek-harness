 ...
 * handoff. The model and shell retain the clean URL. App command-line values
 * arrive through the `rxlabStartup` service expressions in the bundle patch.
 * @module @deepseek-ai/dsh-rxlab-app
 */
...
export const name = 'rxlab-app'

/** This dsh installation's root, from either this package's source or built entry. */
const SOURCE_ROOT = fileURLToPath(new URL('../../../..', import.meta.url))
const ANNOUNCED_ROOTS = new WeakSet<Context>()

/** Runtime service that releases rxlab rows after bind-dependent values resolve. */
const RUNTIME_SERVICE = 'rxlabRuntime'

/** Services required before the rxlab runtime can mount. */
...
export interface Config {
  ...
  printUrl: boolean
  /**
   * Register the model-visible surface context (the `app:rxlab-surface` prompt
   * section and the `DSH_RXLAB_URL` bash variable). A one-shot non-interactive
...
/** Bind-dependent rxlab runtime values shared by the trust fence and URL display. */
export interface RxlabRuntimeValues {
...
/** Environment variable naming the canonical local URL of this rxlab GUI. */
const DSH_RXLAB_URL = 'DSH_RXLAB_URL' as const
 ...
 */
export function resolveLanTrust(bindHost: string, extra: readonly string[]): RxlabRuntimeValues {
...
function localRxlabUrl(ctx: Context): string {
  const port = ctx.get('webServer')?.port
  if (port === undefined) throw new Error('rxlab-app: webServer service missing while resolving the rxlab runtime')
...
/**
 * Mount the rxlab runtime: dist serving, surface prompt, the bash runtime
  ...
  if (config.surfaceContext) {
    ctx.inject(['systemPrompt'], (promptCtx) => {
      ...
      promptCtx.systemPrompt.section({
        name: 'app:rxlab-surface',
        order: promptCtx.systemPrompt.getSectionOrder('WEB_SURFACE'),
        text: () => rxlabSurfacePrompt(localRxlabUrl(promptCtx)),
      })
    })
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'rxlab-runtime',
        variables: {
          [DSH_RXLAB_URL]: { description: 'Canonical local URL of the DeepSeek Harness rxlab GUI serving this session.' },
        },
        resolve: () => ({ [DSH_RXLAB_URL]: localRxlabUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl || handoffBrowser) {
    ctx.inject(['connection'], (connectionCtx) => {
      ...
      const announceReady = (): void => {
        ...
        const webUrl = localRxlabUrl(connectionCtx)
        ...
        if (config.printUrl) {
          console.log(`dsh rxlab: ${authenticatedUrl}${lanUrl === undefined ? '' : ` (LAN: ${lanUrl})`}`)
        }
        if (handoffBrowser) {
          console.log('dsh rxlab: opening the default browser; pass --no-open to disable')
          void internals.openBrowser(authenticatedUrl).catch((error: unknown) => {
            const reason = error instanceof Error ? error.message : String(error)
            console.error(`rxlab-app: could not open the default browser because ${reason}; use the dsh rxlab URL printed at startup`)
          })
        }
      }
