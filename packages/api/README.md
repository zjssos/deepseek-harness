...
| [`rxlab-backup/`](rxlab-backup/README.md) | Packages and restores the workspace's `rxlab_*` business storage tree as one JSON bundle over the typed `rxlabBackup` Remote; a rxlab business row that self-mounts its Client contribution. | `ctx.backupController` / `ctx.remote.rxlabBackup` |
...
| [`rxlab-content/`](rxlab-content/README.md) | Owns the rxlab content assets: stage-tagged knowledge entries and script templates over the `rxlab_content` storage domain; a rxlab business row that self-mounts its Client contribution. | `ctx.contentController` / `ctx.remote.rxlabContent` |
...
| [`rxlab-job/`](rxlab-job/README.md) | Owns the rxlab work orders: jobs, six-stage artifacts, and generated guides over the `rxlab_job` storage domain, plus the deterministic guide assembler; a rxlab business row that self-mounts its Client contribution. | `ctx.jobController` / `ctx.remote.rxlabJob` |
| [`rxlab-recommend/`](rxlab-recommend/README.md) | Owns the rxlab deterministic candidate engine: frame/lens compatibility and ranking over the fitting prescription and advice, exposed as the typed `rxlabRecommend` Remote; a rxlab business row that self-mounts its Client contribution. | `ctx.recommendController` / `ctx.remote.rxlabRecommend` |
| [`rxlab-usage/`](rxlab-usage/README.md) | Owns rxlab token accounting: per-session and per-module totals over the token-meter projection, exposed as the typed `rxlabUsage` Remote; a rxlab business row that self-mounts its Client contribution. | `ctx.usageController` / `ctx.remote.rxlabUsage` |
