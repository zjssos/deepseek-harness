...
The `./tools` row registers model-facing tools and one `tool:browser` system-prompt section; both are per-session contributions that ride the `collect` preset (or any composition mounting that row). The deterministic run loop stays token-free. When only the base `rxlab-collect` row is mounted, nothing registers: no prompt, tool, or session event, and captures run without a model key.

#### KV Cache effect

The mounted tool schemas and the `tool:browser` section enter every model request of a session composed on the `collect` preset; collector mutations do not alter model requests.
