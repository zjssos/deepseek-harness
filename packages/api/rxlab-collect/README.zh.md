...
`./tools` 行注册模型侧工具与一个 `tool:browser` 系统提示分区;两者都是随 `collect` preset(或任何挂载该行的组装)进入的会话级贡献。确定性运行链路保持零 token。仅挂载基础 `rxlab-collect` 行时不注册任何东西:没有 prompt、工具或会话事件,抓取无需模型 key。

#### KV Cache 影响

组装在 `collect` preset 上的会话,其每次模型请求都携带挂载的工具 schema 与 `tool:browser` 分区;采集变更不改变模型请求。
